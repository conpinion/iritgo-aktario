		...
		try
		{
			SwingUtilities.invokeAndWait (new Runnable ()
			{
				public void run ()
				{
					...
					for (Iterator i = controller.getWidgetDescriptions ().iterator (); i.hasNext ();)
					{
						...
						if (wd.getRendererId ().equals ("group"))
						{
							...
							panel.add (groupPanel, createConstraints (0, y, 3, 1, 1.0, 0.0,
											GridBagConstraints.HORIZONTAL, GridBagConstraints.NORTHWEST, new Insets (
															12, 8, 12, 8)));
							++y;
					...
					if (props.getProperty ("surroundingcommands", "no").equals ("yes"))
					{
										...
										dataObjectGUIPane.getOnScreenUniqueId (), "surroundingcommands"))
						{
											...
											.hasNext ();)
							{
								...
								{
									for (Iterator i = Client.instance ().getGUIExtensionManager ()
													.getExtensionIterator (dataObjectGUIPane.getOnScreenUniqueId (),
																	"toolbar"); i.hasNext ();)
									{
					...
					((JPanel) content).add (scrollPane, createConstraints (0, 0, 1, 1, 1.0, 1.0,
									GridBagConstraints.BOTH, GridBagConstraints.NORTHWEST, null));
